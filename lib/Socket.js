      ...
      postAuthenticate: (socket, data) => {
        ...
        iostream(socket).on('upload', (stream,data) => {
          ...
          Promise.all([
            ...
            message = new Message(SUCCESS, EVENT.upload, `文件${name}上传成功`);
          }).catch(err => {
            log.error(err);
            message = new Message(FAILUE, EVENT.upload, `文件${name}上传失败：${err.message}`);
          }).finally(()=>{
            socket.emit('message', JSON.stringify(message));
            return new Log({
              client: socket.client._id,
        ...
        iostream(socket).on('download', (outputStream, data) => {
          let {name,type} = data;
          let message = {};
          Promise.resolve(oss.getStream(`${type}/${name}`)).then(response => {
            message = new Message(SUCCESS, EVENT.download, `文件${name}下载成功`);
            response.stream.pipe(outputStream);
          }).catch(err => {
            log.error(err);
            message = new Message(FAILUE, EVENT.download, `文件${name}上传失败：${err.message}`);
          }).finally(()=>{
            socket.emit('message', JSON.stringify(message));
            return new Log({
              client: socket.client._id,
